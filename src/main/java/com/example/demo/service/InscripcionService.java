...
public class InscripcionService {
    ...
    public void probarConsultas() {
        ...
        System.out
                .println(CollectionsFormatter.toStringList(inscripcionRepo.findAllWhereEstadoIsPendienteOrRechazada()));
        ...
        System.out.println(
                CollectionsFormatter.toStringList(inscripcionRepo.findAllWhereEstadoIs(Inscripcion.Estado.ACEPTADA)));

        System.out.println("inscripcionRepo.findByEstadoIs(Inscripcion.Estado.PENDIENTE)");
        System.out.println(
                CollectionsFormatter.toStringList(inscripcionRepo.findByEstadoIs(Inscripcion.Estado.PENDIENTE)));
    ...
    @Transactional
    public void agregarInscripcion(@NotNull @Positive Long curso_id, @NotNull @Positive Long estudiante_id) throws WrongIdException {

        Curso curso = cursoRepo.findById(curso_id).orElseThrow(() -> new WrongIdException("Curso"));
        Estudiante estudiante = estudianteRepo.findById(estudiante_id).orElseThrow(() -> new WrongIdException("Estudiante"));

        if (estudiante.getEdad()<18) {
            throw new RuntimeException("Age not allowed");
        }

        Inscripcion inscripcion = new Inscripcion(null, curso, estudiante, LocalDate.now(), Inscripcion.Estado.PENDIENTE);
